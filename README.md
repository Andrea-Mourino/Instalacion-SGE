...
Para saber si a funcionado correctamente buscamos en el navegador: 
```http://localhost```
...
Para berificar que los cambios funcionan hacemos un ```sudo systemctl status mysql``` y deberia de verse algo tal que así
...
Instalamos php y el modulo para que apache pueda interpretar php y vemos si esta corriendo correctamente
...
Le damos \q para salir de mysql. Acto seguido instalamos phpmyadmin con ```sudo apt install phpmyadmin``` y deberia salir una ventana tal que asi
...
Aqui comprobamos que se a instalado correctamente y luego crear un enlace para poder acceder desde el navegador y comprobamos que se ha creado
...
Copiamos el archivo de configuracion de ejemplo y editamos el archivo de configuracion
...
Y agregamos el nombre de la base de datos, el user y la contraseña
...
Guardamos y salimos con ctrl + x y luego Y. Escribimos los datos de la base de datos que hemos creado antes
...
Seguidamemte copiamos las lineas que nos da y las pegamos en el archivo de configuracion, reemplazando las que ya hay
